...
import 'package:residencial_cocoon/Controladores/controllerVistaVisualizarPrescripcionesControl.dart';
...
class VistaVisualizarPrescripcionesControl extends StatefulWidget {
  @override
  State<VistaVisualizarPrescripcionesControl> createState() => _VistaVisualizarPrescripcionesControlState();
}

class _VistaVisualizarPrescripcionesControlState extends State<VistaVisualizarPrescripcionesControl> implements IvistaVisualizarPrescripcionesControl {
  Future<List<PrescripcionDeControl>> _prescripciones = Future.value([]);
  ControllerVistaVisualizarPrescripcionesControl _controller = ControllerVistaVisualizarPrescripcionesControl.empty();
  int _paginaActual = 1;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text(
          'Prescripciones de Controles Médicos',
  ...
  Widget buildWideLayout() {
    return Column(
      children: [
        ...
        Expanded(
          child: FutureBuilder<List<PrescripcionDeControl>>(
            future: _prescripciones,
            builder: (BuildContext context, AsyncSnapshot<List<PrescripcionDeControl>> snapshot) {
                ...
                if (snapshot.data!.isEmpty) {
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      PrescripcionDeControl prescripcion = snapshot.data![index];
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(height: 8.0),
                                    Text(
                                      prescripcion.fecha_desde != DateTime(0) ? 'Fecha desde: ${DateFormat('dd/MM/yyyy').format(prescripcion.fecha_desde)}' : 'Fecha desde: ---',
                                      style: const TextStyle(fontSize: 14.0),
                                    ),
                                    const SizedBox(height: 8.0),
                                    Text(
                                      prescripcion.fecha_hasta != DateTime(0) ? 'Fecha Hasta: ${DateFormat('dd/MM/yyyy').format(prescripcion.fecha_hasta)}' : 'Fecha Hasta: ---',
                                      style: const TextStyle(fontSize: 14.0),
                                    ),
                                    const SizedBox(height: 8.0),
                                    Text(
  ...
  void mostrarPopUp(PrescripcionDeControl prescripcion) {
    showDialog(
      context: context,
